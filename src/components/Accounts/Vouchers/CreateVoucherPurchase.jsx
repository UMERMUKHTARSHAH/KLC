...
const CreateVoucherPurchase = () => {
  ...
  const { supplierId = '', orders = [] } = location.state || {};
  ...
  const getLedgerId = async () => {
    try {
      const response = await fetch(
        `${GET_LEDGERSupplierId__URL}/${supplierId}`,
        {
          method: 'GET',
          headers: {
  ...
  const gstLedgerOptions =
    LedgerIncome?.filter((ledg) =>
  ...
  const determineDestinationLedger = (voucherData, destOptions) => {
    ...
    return (
      ...
      destOptions.find((opt) => opt.label?.toLowerCase().includes(baseType))
        ?.value ||
      null
    );
  };
  ...
  const GetVoucherNos = async () => {
    try {
      ...
      console.log(
        data,
        '4444444444444444444444444444444444444444444444444444444444448',
      );
  ...
  const handleOrderSelect = async (selectedValues) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/order/order-products/by-order-ids?orderIds=${orderIdsParam}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const getAvailableProductsForRow = (values, currentIndex) => {
    ...
    return availableProducts.filter(
      (product) => !usedProductIds.includes(product.value),
    );
  };
  ...
  const validationSchema = Yup.object().shape({
    ...
    supplierInvoiceNumber: Yup.string().required(
    ...
    selectedOrderIds.forEach((orderId) => {
      ...
      if (Array.isArray(order.products) && order.products.length > 0) {
        order.products.forEach((orderProduct) => {
          ...
          const rateCalc = calculatePurchaseRate(
            mrp,
            gstRate,
            isRegularSupplier,
          );
        ...
        <Formik
          ...
          onSubmit={async (values, { setSubmitting }) => {
            ...
            const payload = {
              ...
              gstAmount: isRegularSupplier
                ? parseFloat(totals.totalGSTAmount)
                : 0,
              totalIgst: isRegularSupplier
                ? parseFloat(totals.totalGSTAmount)
                : 0,
              totalSgst: isRegularSupplier
                ? parseFloat(totals.totalGSTAmount)
                : 0,
              totalCgst: isRegularSupplier
                ? parseFloat(totals.totalGSTAmount)
                : 0,
        ...
        >
          {({ isSubmitting, setFieldValue, values }) => {
            ...
            useEffect(() => {
              ...
              const updatedPaymentDetails = values.paymentDetails.map(
                (entry) => {
                  const mrp = entry.mrp || 0;
                  const gstRate = entry.gstRate || 0;
                  const quantity = entry.quantity || 1;
                  const rateCalculation = calculatePurchaseRate(
                    mrp,
                    gstRate,
                    isRegularSupplier,
                  );
            ...
            useEffect(() => {
              if (
                Vouchers?.typeOfVoucher?.toLowerCase() === 'purchase' &&
            ...
            const handleOrdersChange = (selectedOptions) => {
              ...
              setFieldValue(
                'paymentDetails',
                values.paymentDetails.filter((pd) =>
                  selectedValues.includes(pd.orderId),
                ),
              );
                    ...
                    <div className="flex flex-col p-6.5">
                      ...
                      <div className="flex flex-row gap-4 mb-6">
                        ...
                        <div className="flex-2 min-w-[200px]">
                          ...
                          <ReactSelect
                            ...
                            value={orders
                              .filter((ord) =>
                                selectedOrderIds.includes(ord?.orderId),
                              )
                              .map((ord) => ({
                                value: ord.orderId,
                                label: ord.orderNo,
                        ...
                        {isRegularSupplier && (
                          <div className="flex-2 min-w-[200px]">
                            ...
                            <ReactSelect
                              name="gstLedgerId"
                              value={
                                gstLedgerOptions?.find(
                                  (opt) => opt.value === values.gstLedgerId,
                                ) || null
                              }
                        ...
                        (isRegularSupplier ? (
                          <div className="mt-4 mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded">
                            <p className="text-blue-700 dark:text-blue-400 text-sm">
                              ℹ️ Regular registered supplier — GST is stripped
                              from MRP, and the GST amount is posted to the
                              Input GST ledger (ITC eligible). Supplier is owed
                              the full MRP.
                            </p>
                          </div>
                        ) : (
                          <div className="mt-4 mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded">
                            <p className="text-yellow-700 dark:text-yellow-400 text-sm">
                              ...
                              {regType?.toUpperCase()}. No GST is applicable —
                              rate equals MRP, and nothing is posted to any GST
                              ledger.
                            </p>
                          </div>
                        ))}
                              ...
                              <table className="w-full table-fixed border-collapse">
                                <thead>
                                  <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                    <th className="w-[200px] py-4 px-3 font-medium text-black dark:text-white">
                                      Product
                                    </th>
                                    <th className="w-[100px] py-4 px-3 font-medium text-black dark:text-white">
                                      Quantity
                                    </th>
                                    <th className="w-[120px] py-4 px-3 font-medium text-black dark:text-white">
                                      MRP (Inc. GST)
                                    </th>
                                    <th className="w-[140px] py-4 px-3 font-medium text-black dark:text-white">
                                      Rate (Excl. GST)
                                    </th>
                                    <th className="w-[100px] py-4 px-3 font-medium text-black dark:text-white">
                                      GST Amount
                                    ...
                                    <th className="w-[80px] py-4 px-3 font-medium text-black dark:text-white">
                                      Action
                                    </th>
                                  </tr>
                                </thead>
                                        ...
                                        <tr key={entry.id || index}>
                                          ...
                                          <td className="border-b py-4 px-3">
                                            <Field
                                              ...
                                              readOnly
                                              className="w-full bg-gray-50 dark:bg-slate-800 py-2 px-3 text-sm rounded border font-medium"
                                            />
                                          </td>
                      ...
                      <div className="flex justify-center mt-4">
                        <button
                          ...
                          className="flex w-[200px] rounded-lg justify-center bg-primary p-3 font-medium text-white hover:bg-opacity-90"
                        >
